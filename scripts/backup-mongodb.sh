#!/bin/bash
# ==============================================================================
# MongoDB Backup Script for CRM Kilang Desa Murni Batik
# ==============================================================================
# This script performs automated backups of MongoDB databases with:
# - BSON archive compression
...
# Databases to backup
DATABASES="${DATABASES:-crm_customers}"
...
check_dependencies() {
    local deps=(mongodump mongorestore)
    for dep in "${deps[@]}"; do
...
backup_database() {
    local db_name="$1"
    local backup_file="${BACKUP_DIR}/daily/${db_name}_${DATE_FORMAT}.archive"
    
    log_info "Starting backup of database: ${db_name}"
    
    local mongo_uri=$(get_mongo_uri "${db_name}")
    
    # Create compressed archive dump
    mongodump \
        --uri="${mongo_uri}" \
        --archive="${backup_file}" \
        --gzip \
        2>&1 | grep -v "^mongodump" || true
...
verify_backup() {
    ...
    # Check archive integrity by listing contents
...
rotate_backups() {
    ...
    find "${BACKUP_DIR}/daily" -name "*.archive" -mtime +${RETENTION_DAILY} -delete 2>/dev/null || true
    
    # Weekly backups (Sunday)
    local day_of_week=$(date +%u)
    if [[ "${day_of_week}" == "7" ]]; then
        for file in "${BACKUP_DIR}"/daily/*_${DATE_FORMAT%%_*}*.archive; do
            if [[ -f "${file}" ]]; then
    ...
    # Weekly retention
    find "${BACKUP_DIR}/weekly" -name "*.archive" -mtime +$((RETENTION_WEEKLY * 7)) -delete 2>/dev/null || true
    ...
    # Monthly retention
    find "${BACKUP_DIR}/monthly" -name "*.archive" -mtime +$((RETENTION_MONTHLY * 30)) -delete 2>/dev/null || true
...
main() {
    ...
    if [[ ${fail_count} -gt 0 ]]; then
        send_notification "error" "MongoDB backup completed with ${fail_count} failures"
        exit 1
    else
        send_notification "success" "MongoDB backup completed successfully (${success_count} databases)"
        exit 0
    fi
}
